<script lang='ts'>
  ...
  function moveLogosBottom(index: number) {
    const currentlyLearningParent = document.getElementsByClassName(`current-logo-${index}`)[0]
    const movingLogo = document.getElementsByClassName(`moving-logo-container-${index}`)[0]
  ...
  onMount(() => {
    ...
    logos.forEach((_, index) => {
      ...
      if (!crystalBallRect || !curLogoRect)
        return
   ...
   */
  function updateRadius(windowWidth: number): { horizontalRadius: number, verticalRadius: number } {
   ...
   */
  function getLogoPosition(index: number, windowWidth: number, windowHeight: number) {
    if (windowWidth <= 768) {
      const circlePosition = calculateLogoPositionThreeRow(Math.floor(index / 3), windowWidth, windowHeight, index % 3)
      return `${circlePosition}; width: ${logoSize}px;`
    }
    const isTopCircle = index % 2 === 1
    const circlePosition = calculateLogoPositionTwoRow(Math.floor(index / 2), windowWidth, windowHeight, isTopCircle)
    return `${circlePosition}; width: ${logoSize}px;`
  }
   ...
   *
   */
  function calculateLogoPositionThreeRow(index: number, windowWidth: number, windowHeight: number, rowNum: number) {
    ...
    const x = Math.cos(angle) * (horizontalRadius + horizontalOffset) + (windowWidth / 2) - logoSize / 2
    const y
      = Math.sin(angle) * -(verticalRadius + verticalOffset) + (windowHeight / heightOffset)
   ...
   *
   */
  function calculateLogoPositionTwoRow(index: number, windowWidth: number, windowHeight: number, isTopCircle: boolean) {
    const totalLogos = logos.length
    ...
    // Calculate x and y positions based on oval radii
    const x = Math.cos(angle) * (horizontalRadius + horizontalOffset) + (windowWidth / 2) - logoSize / 1.6
    const y
      = Math.sin(angle) * -(verticalRadius + verticalOffset) + (windowHeight / heightOffset)
