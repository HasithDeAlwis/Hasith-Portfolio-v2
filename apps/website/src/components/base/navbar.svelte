...
<header id='header' class={`fixed top-0 left-0 right-0 z-40 bg-opacity-75  backdrop-blur-md ${window.location.pathname === '/' ? 'bg-primary-400' : 'bg-primary-100'}`}>
    ...
    <nav id='navbar' class='sm:basis-1/2 lg:basis-1/3'>
      <div class='items-center justify-around hidden space-x-4 md:flex'>
        {#each links as { href, text, isHome }}
          <a aria-label={`navigate to ${text} section`} href={window.location.pathname !== '/' && isHome ? `/${href}` : href} class='font-bold text-primary-800 hover:text-primary-900 focus:text-secondary-800 font-body text-p1-mobile md:text-p2-desktop'>
            {text}
          </a>
        {/each}
      </div>
