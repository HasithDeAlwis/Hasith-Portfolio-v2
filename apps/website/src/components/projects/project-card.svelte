  ...
  <div class='h-auto transition-shadow duration-300 basis-2/5 md:shadow-primary-200 md:hover:shadow-primary-200-hover'>
    <div class='relative flex flex-col w-full h-full px-3 pb-4 bg-primary-800'>
      ...
      <div class='flex justify-start gap-4 mt-5'>
        {#if project.demoLink}
          <Button href={project.demoLink} target='_blank' rel='noopener noreferrer' variant='default'>Demo</Button>
        {/if}
        {#if project.readMoreLink}
          <Button href={project.readMoreLink} target='_blank' rel='noopener noreferrer' variant='default'>Read More</Button>
        {/if}
        {#if project.sourceLink}
          <Button href={project.sourceLink} target='_blank' rel='noopener noreferrer' variant='default'>GitHub</Button>
        {/if}
      </div>
    </div>
  </div>
